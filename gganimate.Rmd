...
Shadows can add a nice effect to show paths over some index, especially time.  The following recreates the famous TED talk visualization, which attempted to show how much better things were getting.
...
p +
  transition_time(year) +
  shadow_trail(distance = .02, alpha = 0.3, shape = 2)
```

Maybe things hadn't changed that much after all for some, relatively speaking.
...
Animation is a great way to enhance a plot and tell a data driven story.  It can definitely be overkill for many situations though, and so should be used with caution.