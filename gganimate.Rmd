...
Note that *all animations will take at least several seconds to produce*.  To reduce this you can change the number of frames used, though this will typically make for a choppier plot. Note that output will be forced to the viewer instead of inline also.
...
p = film_timeline + 
  transition_time(release_date) 

animate(p, end_pause = 25)
```

See how this contrasts with `transition_reveal`
...
p = mass_height +
  transition_reveal(seq_along(mass)) 
